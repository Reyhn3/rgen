...
//TODO: Add tests to assert a reasonable statistically distributed random confidence
	...
	{
		var result = _sut.Generate(1, 1, A.Dummy<IntegerGeneratorOptions>());
	...
	{
		var result = _sut.Generate(1, 1, A.Dummy<IntegerGeneratorOptions>()).ToArray();
	...
	{
		var result = _sut.Generate(1, 2, A.Dummy<IntegerGeneratorOptions>()).ToArray();
		GeneratorUtils.PrintSets(result);
	...
	{
		var result = _sut.Generate(100, 1, A.Dummy<IntegerGeneratorOptions>()).ToArray();
		GeneratorUtils.PrintSets(result);
	...
	{
		var result = _sut.Generate(100, 100, A.Dummy<IntegerGeneratorOptions>()).ToArray();
		GeneratorUtils.PrintSets(result);
